...
<div class="layout">
    ...
    <div id="propertie_add">
    	<label class="label">Название</label><input type="text" name="propertie_title" value="{if isset($array.POST.propertie_title)}{$array.POST.propertie_title}{else}{$array.moduleCatalogObjectPropertie.propertie_title}{/if}"{if isset($array.errors.propertie_title_error)} class="error"{/if} />
    	<div class="clearfix"></div>
    	...
    	<div class="select">   
            ...
            {foreach from=$array.moduleCatalogType key=key item=title}
                <option value="{$key}"{if $key == $array.moduleCatalogObjectPropertie.type_id} selected="selected"{/if}>{$title}</option>
            {/foreach}
            </select>   
        </div>
        <div class="clearfix"></div>  
    ...
    {if $array.propertiearraylist && $array.moduleCatalogObjectPropertie.type_id == 3}
    	...
    	<table>
    		...
    		<tbody>
                ...
                <tr {if $smarty.foreach.propertiearraylist.index % 2 == 1}class="f"{/if}>
                    ...
                    <td>
                        <a href="{$SITE_URL}module-catalog-objects-propertie-array_form.php?catalog_id={$array.catalog_id}&category_id={$array.category_id}&propertie_id={$array.propertie_id}&array_id={$array_field.array_id}">Редактировать</a> /
                        <a href="javascript:;" onclick="deleteMe('{$SITE_URL}module-catalog-objects-propertie_form.php?catalog_id={$array.catalog_id}&category_id={$array.category_id}&object_id={$array.object_id}&propertie_id={$array.propertie_id}&_array_id={$array_field.array_id}','Вы уверены, что хотите удалить поле?');">Удалить</a>
                    </td>
                </tr>
                {/foreach}        
            </tbody>
